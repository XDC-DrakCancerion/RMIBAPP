...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($mode === 'delete') {
    ...
    try {
      db()->prepare("DELETE FROM tbl_pekerjaan WHERE id_pekerjaan = ?")
        ->execute([$idp]);
  ...
  if ($mode === 'Tambah') {
    ...
    if (!in_array($kode_kelompok, $steps, true)) {
      $kode_kelompok = 'A';
    }
    ...
    try {
      $ins = db()->prepare("
        INSERT INTO tbl_pekerjaan 
        (id_kelompok, jenis_kelamin, nama_pekerjaan)
        VALUES (?, ?, ?)
      ");
      ...
      flash_set(
        'success',
        "Tambah pekerjaan berhasil untuk Kelompok {$kode_kelompok} JK {$jk_post}. Silakan mapping kategori untuk setiap pekerjaan."
      );
  ...
  <main class="flex-1 p-6">
    ...
    <div class="flex items-center justify-between gap-4 mb-6">
      ...
      <div class="flex gap-2">
        <a class="px-4 py-2 rounded-xl border bg-white hover:bg-slate-50"
           href="dashboard.php">
          Dashboard
        </a>
    ...
    <?php if ($action === 'list'): ?>
      <!-- FILTER BAR HANYA MUNCUL DI HALAMAN LIST -->
        ...
        <form method="get" class="grid md:grid-cols-4 gap-3">
          ...
          <div>
            ...
            <select name="jk" class="mt-1 w-full px-3 py-3 rounded-xl border bg-white">
              <option value="L" <?= $jk === 'L' ? 'selected' : '' ?>>
                L (Laki-laki)
              </option>
              <option value="P" <?= $jk === 'P' ? 'selected' : '' ?>>
                P (Perempuan)
              </option>
            </select>
          </div>
          ...
          <div class="md:col-span-2">
            ...
            <div class="mt-1 flex gap-2">
              ...
              <button
                type="submit"
                class="px-5 py-3 rounded-xl bg-slate-900 text-white font-semibold hover:opacity-95">
                Filter
              </button>
            </div>
          </div>
        ...
        <form method="post" class="space-y-4">
          ...
          <div>
            ...
            <select name="id_kategori"
                    class="mt-1 w-full px-4 py-3 rounded-xl border bg-white"
                    required>
              <option value="0" <?= !$editData['id_kategori'] ? 'selected' : '' ?>>
                -- Pilih Kategori --
              </option>
            ...
            <div class="text-xs text-slate-500 mt-1">
              Pilih kategori yang paling sesuai dengan pekerjaan ini.
            </div>
          </div>
          ...
          <div class="flex gap-2">
            <button
              type="submit"
              class="px-6 py-3 rounded-xl bg-slate-900 text-white font-semibold hover:opacity-95">
              Simpan Mapping
            </button>

            <a class="px-6 py-3 rounded-xl border bg-white hover:bg-slate-50"
               href="pekerjaan.php?k=<?= e($k) ?>&jk=<?= e($jk) ?>">
              Batal
            </a>
          </div>
        </form>
              ...
              <?php if (!$rows): ?>
                <tr>
                  <td colspan="5" class="py-4 text-slate-500">
                    Data kosong. Tambahkan pekerjaan dengan tombol Tambah.
                  </td>
                </tr>
              <?php endif; ?>

              <?php foreach ($rows as $i => $r): ?>
                <?php $idKat = (int)($r['id_kategori'] ?? 0); ?>
                ...
                <tr class="border-b last:border-b-0">
                  ...
                  <td class="py-3 pr-4">
                    <?php if ($idKat > 0 && isset($categories[$idKat])): ?>
                  ...
                  <td class="py-3 pr-4">
                    <div class="flex gap-2">
                      ...
                      <form method="post"
                        ...
                        <button
                          type="submit"
                          class="px-3 py-2 rounded-xl bg-red-600 text-white hover:opacity-95 text-xs">
                          Hapus
                        </button>
                      </form>
                    </div>
