...
<div class="flex">
  ...
  <main class="flex-1 p-6">
   <h1 class="flex items-center gap-2 text-2xl font-semibold">
    ...
    <div class="bg-white border rounded-2xl p-6 space-y-4 text-black">
      <?php foreach ($rows as $r): ?>
        <div class="p-4 rounded-2xl border hover:bg-slate-50 text-black">
          <div class="font-semibold text-black">
            <?= (int)$r['id_kategori'] ?>. <?= e($r['nama_kategori']) ?>
            <span class="text-black">(<?= e($r['kd_kategori']) ?>)</span>
          </div>
          <div class="text-black mt-1">
            <?= e($r['deskripsi_kategori'] ?? '-') ?>
          </div>
        </div>
      <?php endforeach; ?>
    </div>
